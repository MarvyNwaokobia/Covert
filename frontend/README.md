...
- See aggregate cost of payroll — but cannot reconstruct individual salaries once submitted
...
If you navigate to a dashboard that doesn't match your role, `RoleGuard` redirects you to `/unauthorized`.
...
1. The frontend calls `useFhevm().generateDecryptionKeypair()` — generates an ephemeral keypair in the browser
...
3. The user signs the message with MetaMask — this proves they control the wallet without sending any transaction
...
2. This calls `fhevmjs`'s `createEncryptedInput(contractAddress, employerAddress)` — binding the ciphertext to this specific contract and sender
3. `input.add64(value)` encrypts the value as a `euint64`
4. `input.encrypt()` returns a `{ handles, inputProof }` pair — the handle is the on-chain ciphertext reference, the proof proves the encryption was formed correctly
...
- **F-04** — Distribution trigger with gas estimation + transaction feedback
- **F-05** — Employee payslip with EIP-712 decrypt reveal animation
- **F-06** — Employee peer bonus allocation UI
...
The contract is deployed to Sepolia testnet. Once available:

1. Copy the deployed contract address
2. Set `NEXT_PUBLIC_CONTRACT_ADDRESS` in `.env.local`
3. Replace `src/lib/abi/CovertPayroll.json` with the real exported ABI
...
### fhevmjs Deprecation
`fhevmjs@0.6.2` is currently deprecated in favour of `@zama-fhe/relayer-sdk`. We are using `fhevmjs` because the new package introduces a wagmi v3 dependency that conflicts with RainbowKit v2. Migration to `@zama-fhe/relayer-sdk` is planned once RainbowKit releases wagmi v3 support.
