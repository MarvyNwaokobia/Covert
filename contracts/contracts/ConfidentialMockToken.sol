 ...
 * @notice A minimal, self-contained ERC-7984-style confidential fungible token used as Covert's
 *         cUSDT for LOCAL tests and the demo faucet. Balances are `euint64` ciphertexts; transfers
 *         move encrypted amounts; approvals use the time-bounded operator model.
 * @dev Self-contained on purpose: it depends only on `@fhevm/solidity` (not on a specific
 *      openzeppelin/confidential-contracts version), so Covert's `IConfToken` usage is guaranteed
 *      to match. On Sepolia you may instead point Covert at the OFFICIAL cUSDT mock
 *      (0x4E7B06D78965594eB5EF5414c357ca21E1554491) via `CUSDT_ADDRESS`; this mock is for local
 *      Hardhat runs and self-contained Sepolia demos.
 */
contract ConfidentialMockToken is IConfToken, ZamaEthereumConfig {
    ...
    /// @notice Faucet: mint a cleartext amount as a confidential balance (demo only).
    ...
    function confidentialTransfer(address to, externalEuint64 enc, bytes calldata inputProof)
        external
        returns (euint64)
    {
    ...
    /**
     * @dev Confidential transfer: clamps to the sender's balance (never reverts on insufficient
     *      funds — ERC-7984 semantics; the actual moved amount stays encrypted).
     */
    function _transfer(address from, address to, euint64 amount) internal returns (euint64 sent) {
        ...
        // ACL on the moved amount goes to the actual counterparties only.
        FHE.allowThis(sent);
        FHE.allow(sent, from);
        FHE.allow(sent, to);
        return sent;
